...
from nextcord.ext.commands import check,Context, context
import utilities
import asyncio

create_channel_aliases = ["Create","CreateJoinForVC","newChannel","add_channel","add"]
class voicechannels(commands.Cog):
    ...
    async def createChannel(self, ctx: commands.context,*,text = "Join for a new VC"):
        channel = await nextcord.Guild.create_voice_channel(ctx.channel.guild, name = text, overwrites=None, bitrate=8000,reason="Channel to create new.")
    ...
    async def on_voice_state_update(self, member,before,after):
        ...
        if after.channel and after.channel.bitrate == 8000 and str(after.channel.id) in joinForChannels: #These ensure its the correct channel
            channel_name = str(utilities.getChannelName(member))
            duplicate_detector = nextcord.utils.get(after.channel.guild.voice_channels, name=channel_name)
            if duplicate_detector:
                await member.move_to(duplicate_detector)
            else:                            
                #channel_info = {'guild':after.channel.guild,'overwrites':None,'category':after.channel.category,'reason':"Channel For Vibes",'bitrate':utilities.getMaxBitRate(after.channel.guild)}

                channel = await Guild.create_voice_channel(after.channel.guild,name=channel_name, overwrites = {}, category = after.channel.category, reason = "Channel For Vibes", bitrate = utilities.getMaxBitRate(after.channel.guild))#guild = after.channel.guild,
        ...
        if before.channel != None and len(before.channel.members) == 0:            
            ...
            if str(before.channel.id) in temporary_channels:
                ...
                utilities.removeFromFile(temporary_channels,r"data/temporary_channels.csv",str(before.channel.id))


    @commands.Cog.listener()
    async def on_guild_channel_delete(self, channel):
        temporary_channels = utilities.readFileToList(r"data/temporary_channels.csv")
        if str(channel.id) in temporary_channels:
            utilities.removeFromFile(temporary_channels,r"data/temporary_channels.csv",str(channel.id))
